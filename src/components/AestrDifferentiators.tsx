...
const differentiators = [
    {
        title: "2x High-Package Career Pathways",
        points: ["Unlock opportunities for 30 LPA+ roles in top-tier companies", "Structured pathways for global placements", "Exposure to industry leaders like Google, NVIDIA, and Amazon", "Build in-demand skills that command high-paying offers"],
        actionText: "Unlock opportunities for 30 LPA+ roles"
    },
    {
        title: "Future-Proof AI Careers",
        points: ["Designed for careers of the next decade and beyond", "AI applications across FinTech, Robotics, and Healthcare", "Prepare for jobs that will shape the future", "Train for roles that don’t even exist yet"],
        actionText: "AI Jobs of 2030"
    },
    {
        title: "Don't Study AI — Build It",
        points: ["Start building from Day 1", "Move beyond theory-focused learning", "build real-world data pipelines and AI models", "Solve practical AI challenges every day"],
        actionText: "Build from Day 1"
    },
    {
        title: "In Collaboration with Shodh AI",
        points: ["Industry-integrated, campus-driven learning model", "Program designed by the CEO of Shodh AI", "Work alongside experienced AI engineers", "Learn directly from professionals building real-world AI solutions"],
        actionText: "Learn from Builders"
    },
    {
        title: "Silicon Valley Incubator",
        points: ["Go beyond being a student — become a Resident Innovator", "Build your own startup IP from Day 1", "Access to venture funding and expert mentorship", "Guidance from founders associated with Cambridge & Microsoft"],
            ...
            <div className="container-boxed relative z-10">
                ...
                >
                    ...
                    <p className="text-white/70 font-outfit text-base md:text-xl max-w-3xl mx-auto leading-relaxed">
                        Strengthening AESTR’s brand positioning to clearly highlight how it stands apart from traditional B.Tech programs.
                    </p>
                </motion.div>
                    ...
                    {differentiators.map((item, idx) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`relative group p-8 rounded-3xl border border-white/10 overflow-hidden transition-all duration-500 flex flex-col h-full ${isNeon ? 'bg-accent border-accent' : 'bg-black border-white/10'
                                    }`}
                            >
                                ...
                                <div className="relative z-10 flex-grow">
                                    <h3 className={`text-xl md:text-2xl font-orbitron font-bold mb-8 md:mb-10 leading-tight transition-colors min-h-[60px] ${isNeon ? 'text-black' : 'text-white group-hover:text-accent'
                                        }`}>
                                        {item.title}
                                    </h3>
                                    ...
                                    <ul className="space-y-5">
                                        {item.points.map((point, pIdx) => (
                                            <li key={pIdx} className="flex items-start gap-3">
                                                <span className={`w-1.5 h-1.5 rounded-full mt-2 shrink-0 shadow-[0_0_8px_rgba(215,246,1,0.6)] ${isNeon ? 'bg-black' : 'bg-accent'
                                                    }`} />
                                                <span className={`font-outfit text-sm md:text-base leading-snug ${isNeon ? 'text-black/80 font-bold' : 'text-white/80'
                                                    }`}>
                                                    {point}
                                                </span>
                                            </li>
                                ...
                                {/* Action Link Decor - Pushed to bottom with mt-auto */}
                                <div className={`mt-10 pt-6 border-t transition-all duration-300 ${isNeon ? 'border-black/20' : 'border-white/20'
                                    }`}>
                                    <span className={`text-[10px] font-orbitron font-black tracking-widest uppercase transition-all duration-300 ${isNeon
                                        ? 'text-black/60 group-hover:text-black group-hover:scale-105'
                                        : 'text-accent group-hover:text-accent group-hover:scale-105 shadow-[0_0_15px_rgba(215,246,1,0.4)]'
                                        } inline-block`}>
                                        {item.actionText}
                                    </span>
                                </div>
